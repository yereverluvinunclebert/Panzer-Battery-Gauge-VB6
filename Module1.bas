...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hwnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Battery-Gauge-VB6/issues", vbNullString, App.path, 1)
    End If
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    If gblDpiAwareness = "1" Then
        gblFormHighDpiXPosTwips = fGetINISetting("Software\PzBatteryGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblFormHighDpiYPosTwips = fGetINISetting("Software\PzBatteryGauge", "formHighDpiYPosTwips", gblSettingsFile)
...
'        End If
    Else
        gblFormLowDpiXPosTwips = fGetINISetting("Software\PzBatteryGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblFormLowDpiYPosTwips = fGetINISetting("Software\PzBatteryGauge", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPosition()
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            gblFormHighDpiXPosTwips = Str$(panzerPrefs.Left)
            gblFormHighDpiYPosTwips = Str$(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzBatteryGauge", "formHighDpiXPosTwips", gblFormHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzBatteryGauge", "formHighDpiYPosTwips", gblFormHighDpiYPosTwips, gblSettingsFile
        Else
            gblFormLowDpiXPosTwips = Str$(panzerPrefs.Left)
            gblFormLowDpiYPosTwips = Str$(panzerPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzBatteryGauge", "formLowDpiXPosTwips", gblFormLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzBatteryGauge", "formLowDpiYPosTwips", gblFormLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(panzerPrefs.hwnd, "open", thisCommand, "Panzer Battery Gauge.exe prefs", "", 1)
    Else
