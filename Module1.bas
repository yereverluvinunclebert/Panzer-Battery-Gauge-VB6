...
Public Function fLicenceState() As Integer
    ...
    If fFExists(gblSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzBatteryGauge", "licence", gblSettingsFile)
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Battery-Gauge-" & gblCodingEnvironment & "/issues", vbNullString, App.Path, 1)
    End If
...
Public Sub setRichClientTooltips()
   On Error GoTo setRichClientTooltips_Error
    ...
    If gblGaugeTooltips = "1" Then
        ...
        fGauge.gaugeForm.Widgets("housing/stopbutton").Widget.ToolTip = "Press to stop gauge operation."
...
Public Sub readPrefsPosition()
    ...
    If gblDpiAwareness = "1" Then
        gblPrefsHighDpiXPosTwips = fGetINISetting("Software\PzBatteryGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblPrefsHighDpiYPosTwips = fGetINISetting("Software\PzBatteryGauge", "formHighDpiYPosTwips", gblSettingsFile)
    ...
    Else
        gblPrefsLowDpiXPosTwips = fGetINISetting("Software\PzBatteryGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblPrefsLowDpiYPosTwips = fGetINISetting("Software\PzBatteryGauge", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPositionAndSize()
    ...
    If widgetPrefs.IsLoaded = True And widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzBatteryGauge", "formHighDpiXPosTwips", gblPrefsHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzBatteryGauge", "formHighDpiYPosTwips", gblPrefsHighDpiYPosTwips, gblSettingsFile
        Else
            gblPrefsLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblPrefsLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzBatteryGauge", "formLowDpiXPosTwips", gblPrefsLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzBatteryGauge", "formLowDpiYPosTwips", gblPrefsLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hWnd, "open", thisCommand, "Panzer-Battery-Gauge-VB6.exe prefs", "", 1)
    Else
