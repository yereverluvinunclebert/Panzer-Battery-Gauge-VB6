...
Public Sub mainRoutine(ByVal restart As Boolean)
    ...
    ' = "none"
    gblStartupFlg = True
    gblWidgetName = "Panzer Battery Gauge"
    thisPSDFullPath = App.Path & "\Res\Panzer Battery gauge VB6.psd"
    ...
    menuForm.mnuAbout.Caption = "About Panzer Battery Gauge Cairo " & gblCodingEnvironment & " widget"
    ...
    Call readSettingsFile("Software\PzBatteryGauge", gblSettingsFile)
...
Private Sub checkFirstTime()
   ...
   On Error GoTo checkFirstTime_Error
    ...
    If gblFirstTimeRun = "true" Then
        ...
        sPutINISetting "Software\PzBatteryGauge", "firstTimeRun", gblFirstTimeRun, gblSettingsFile
    End If
...
'        gblVolumeBoost = fGetINISetting(Location, "volumeBoost", gblSettingsFile)
        ...
        ' other
        gblGaugeHighDpiXPos = fGetINISetting("Software\PzBatteryGauge", "gaugeHighDpiXPos", gblSettingsFile)
        gblGaugeHighDpiYPos = fGetINISetting("Software\PzBatteryGauge", "gaugeHighDpiYPos", gblSettingsFile)
        gblGaugeLowDpiXPos = fGetINISetting("Software\PzBatteryGauge", "gaugeLowDpiXPos", gblSettingsFile)
        gblGaugeLowDpiYPos = fGetINISetting("Software\PzBatteryGauge", "gaugeLowDpiYPos", gblSettingsFile)
...
Private Sub getToolSettingsFile()
    ...
    gblSettingsDir = fSpecialFolder(feUserAppData) & "\PanzerBattery Gauge" ' just for this user alone
...
Private Sub handleUnhideMode(ByVal thisUnhideMode As String)
    ...
    If thisUnhideMode = "unhide" Then     'parse the command line
        gblUnhide = "true"
        sPutINISetting "Software\PzBatteryGauge", "unhide", gblUnhide, gblSettingsFile
